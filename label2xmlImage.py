import os
import cv2
from xml.dom.minidom import Document

# windows下无需
import sys

stdi, stdo, stde = sys.stdin, sys.stdout, sys.stderr
#reload(sys)
#sys.setdefaultencoding('utf-8')
sys.stdin, sys.stdout, sys.stderr = stdi, stdo, stde

# category_set = ['car', 'sea', 'airplane']


def custombasename(fullname):
    return os.path.basename(os.path.splitext(fullname)[0])


def limit_value(a, b):
    if a < 1:
        a = 1
    if a >= b:
        a = b - 1
    return a


def readlabeltxt(txtpath, height, width, hbb=False):
    print(txtpath)
    with open(txtpath, 'r') as f_in:  # 打开txt文件
        lines = f_in.readlines()
        splitlines = [x.strip().split(' ') for x in lines]  # 根据空格分割
        boxes = []
        for i, splitline in enumerate(splitlines):
            if i in [0, 1]:  # DOTA数据集前两行对于我们来说是无用的
                imageSource = (str(splitlines[0][0])).split(':')[1]
                resolution = (str(splitlines[1][0])).split(':')[1]
                continue
            label = splitline[8]
            dif=splitline[9]
            # if label not in category_set:  # 只书写制定的类别
            #     continue
            x1 = int(float(splitline[0])+0.5)
            y1 = int(float(splitline[1])+0.5)
            x2 = int(float(splitline[2])+0.5)
            y2 = int(float(splitline[3])+0.5)
            x3 = int(float(splitline[4])+0.5)
            y3 = int(float(splitline[5])+0.5)
            x4 = int(float(splitline[6])+0.5)
            y4 = int(float(splitline[7])+0.5)
            # 如果是hbb
            if hbb:
                xx1 = min(x1, x2, x3, x4)
                xx2 = max(x1, x2, x3, x4)
                yy1 = min(y1, y2, y3, y4)
                yy2 = max(y1, y2, y3, y4)

                xx1 = limit_value(xx1, width)
                xx2 = limit_value(xx2, width)
                yy1 = limit_value(yy1, height)
                yy2 = limit_value(yy2, height)

                box = [xx1, yy1, xx2, yy2, label,dif]
                boxes.append(box)
            else:  # 否则是obb
                x1 = limit_value(x1, width)
                y1 = limit_value(y1, height)
                x2 = limit_value(x2, width)
                y2 = limit_value(y2, height)
                x3 = limit_value(x3, width)
                y3 = limit_value(y3, height)
                x4 = limit_value(x4, width)
                y4 = limit_value(y4, height)

                box = [x1, y1, x2, y2, x3, y3, x4, y4, label,dif]
                boxes.append(box)
    return boxes,imageSource,resolution


def writeXml(tmp, imgname, w, h, d, bboxes, hbb=True,pic_path = "null",img_source="null",img_resolution="null",json_path = "null"):
    doc = Document()
    # owner
    AISample = doc.createElement('AISample')
    doc.appendChild(AISample)

   #AILabelPair
    AILabelPair  = doc.createElement('AILabelPair')
    AISample.appendChild(AILabelPair)
   # AILable
   #  AILabel =  doc.createElement('AILabel')
   #  AILabelPair.appendChild(AILabel)

    #object
    for bbox in bboxes:
        # threes#
        AILabel = doc.createElement("AILabel")
        AILabelPair.appendChild(AILabel)
        #featureID
        featureID = doc.createElement('featureID')
        AILabel.appendChild(featureID)
        featureIDTxt = doc.createTextNode(str(bboxes.index(bbox)))
        featureID.appendChild(featureIDTxt)

        #objectName
        objectName = doc.createElement('objectName')
        AILabel.appendChild(objectName)
        objectNameTxt = doc.createTextNode(str(bbox[-2]))
        objectName.appendChild(objectNameTxt)

        #level
        level = doc.createElement('level')
        AILabel.appendChild(level)
        levelTxt = doc.createTextNode("object level")
        level.appendChild(levelTxt)

        # geometry
        geometryType = doc.createElement('geometryType')
        AILabel.appendChild(geometryType)
        geometryTxt = doc.createTextNode("polygon")
        geometryType.appendChild(geometryTxt)

        truncated = doc.createElement('truncated')
        AILabel.appendChild(truncated)
        truncatedTxt = doc.createTextNode("0")
        truncated.appendChild(truncatedTxt)

        difficult = doc.createElement('difficult')
        AILabel.appendChild(difficult)
        difficultTxt = doc.createTextNode(str(bbox[-1]))
        difficult.appendChild(difficultTxt)
        # threes-1#
        bndbox = doc.createElement('bndbox')
        AILabel.appendChild(bndbox)

        if hbb:
            xmin = doc.createElement('xmin')
            bndbox.appendChild(xmin)
            xmin_txt = doc.createTextNode(str(bbox[0]))
            xmin.appendChild(xmin_txt)

            ymin = doc.createElement('ymin')
            bndbox.appendChild(ymin)
            ymin_txt = doc.createTextNode(str(bbox[1]))
            ymin.appendChild(ymin_txt)

            xmax = doc.createElement('xmax')
            bndbox.appendChild(xmax)
            xmax_txt = doc.createTextNode(str(bbox[2]))
            xmax.appendChild(xmax_txt)

            ymax = doc.createElement('ymax')
            bndbox.appendChild(ymax)
            ymax_txt = doc.createTextNode(str(bbox[3]))
            ymax.appendChild(ymax_txt)
        else:
            x0 = doc.createElement('x0')
            bndbox.appendChild(x0)
            x0_txt = doc.createTextNode(str(bbox[0]))
            x0.appendChild(x0_txt)

            y0 = doc.createElement('y0')
            bndbox.appendChild(y0)
            y0_txt = doc.createTextNode(str(bbox[1]))
            y0.appendChild(y0_txt)

            x1 = doc.createElement('x1')
            bndbox.appendChild(x1)
            x1_txt = doc.createTextNode(str(bbox[2]))
            x1.appendChild(x1_txt)

            y1 = doc.createElement('y1')
            bndbox.appendChild(y1)
            y1_txt = doc.createTextNode(str(bbox[3]))
            y1.appendChild(y1_txt)

            x2 = doc.createElement('x2')
            bndbox.appendChild(x2)
            x2_txt = doc.createTextNode(str(bbox[4]))
            x2.appendChild(x2_txt)

            y2 = doc.createElement('y2')
            bndbox.appendChild(y2)
            y2_txt = doc.createTextNode(str(bbox[5]))
            y2.appendChild(y2_txt)

            x3 = doc.createElement('x3')
            bndbox.appendChild(x3)
            x3_txt = doc.createTextNode(str(bbox[6]))
            x3.appendChild(x3_txt)

            y3 = doc.createElement('y3')
            bndbox.appendChild(y3)
            y3_txt = doc.createTextNode(str(bbox[7]))
            y3.appendChild(y3_txt)

    #imageSource
    imageSource = doc.createElement('imageSource')
    # AISample.appendChild(imageSource)
    AILabelPair.appendChild(imageSource)

    imageSourceID = doc.createElement('imageSourceID')
    imageSource.appendChild(imageSourceID)
    imageSourceIDTxt = doc.createTextNode("2")
    imageSourceID.appendChild(imageSourceIDTxt)

    abstract= doc.createElement('abstract')
    imageSource.appendChild( abstract)
    abstractTxt = doc.createTextNode("The image source is original from Google Earth")
    abstract.appendChild(abstractTxt)

    sensorType = doc.createElement('sensorType')
    imageSource.appendChild(sensorType)
    sensorTypeTxt = doc.createTextNode(img_source)
    sensorType.appendChild(sensorTypeTxt)

    format = doc.createElement('format')
    imageSource.appendChild(format)
    formatTxt = doc.createTextNode("png")
    format.appendChild(formatTxt)

    resolution = doc.createElement('resolution')
    imageSource.appendChild(resolution)
    resolutionTxt = doc.createTextNode(img_resolution)
    resolution.appendChild(resolutionTxt)

    width = doc.createElement('width')
    imageSource.appendChild(width)
    widthTxt = doc.createTextNode(str(w))
    width.appendChild(widthTxt)

    height = doc.createElement('height')
    imageSource.appendChild(height)
    heightTxt = doc.createTextNode(str(h))
    height.appendChild(heightTxt)

    imageChannels = doc.createElement('imageChannels')
    imageSource.appendChild(imageChannels)
    imageChannelsTxt = doc.createTextNode(str(d))
    imageChannels.appendChild(imageChannelsTxt)

    imagePath = doc.createElement('imagePath')
    imageSource.appendChild(imagePath)
    imagePathTxt = doc.createTextNode(pic_path)
    imagePath.appendChild(imagePathTxt)

    # Metadata link
    MetadataLink = doc.createElement('MetadataLink')
    # AISample.appendChild(imageSource)
    AILabelPair.appendChild(MetadataLink)

    JsonLink = doc.createElement('JsonLink')
    MetadataLink.appendChild(JsonLink)
    JsonLinkTxt = doc.createTextNode(json_path)
    JsonLink.appendChild(JsonLinkTxt)


    #AI_Task
    AITask = doc.createElement('AITask')
    AISample.appendChild(AITask)

    xmlname = os.path.splitext(imgname)[0]
    tempfile = os.path.join(tmp, xmlname+"_sample" + '.xml')
    with open(tempfile, 'wb') as f:
        f.write(doc.toprettyxml(indent='\t', encoding='utf-8'))
    return


if __name__ == '__main__':
    # data_path = '/home/yantianwang/lala/ship/train/examplesplit'
    # data_path = 'I:\object detection\3_4DOTA\train'
    data_path = r'D:\SampleML\sampleMLV2'
    images_path = os.path.join(data_path, 'image')  # 样本图片路径
    imageName = os.listdir(images_path)
    labeltxt_path = os.path.join(data_path, 'label')  # DOTA标签的所在路径

    anno_new_path = os.path.join(data_path, 'xml')  # 新的voc格式存储位置（hbb形式）
    json_path = os.path.join(data_path,'SampleJson')
    ext = '.png'  # 样本图片的后缀
    filenames = os.listdir(labeltxt_path) # 获取每一个txt的名称
    filenames.sort()
    jsonfileName = os.listdir(json_path)
    jsonfileName.sort()
    for i in range(len(filenames)):
        filepath = labeltxt_path + '/' + filenames[i]  # 每一个DOTA标签的具体路径
        jsonPath = json_path + '/' +jsonfileName[i]
        picname = os.path.splitext(filenames[i])[0] + ext
        pic_path = os.path.join(images_path,picname)
        im = cv2.imread(pic_path)  # 读取相应的图片
        (H, W, D) = im.shape  # 返回样本的大小
        boxes, img_source, img_resolution = readlabeltxt(filepath, H, W, hbb=True)  # 默认是矩形（hbb）得到gt  fnn代表
        if len(boxes) == 0:
            print('文件为空', filepath)
        # 读取对应的样本图片，得到H,W,D用于书写xml
        # 书写xml
        writeXml(anno_new_path, picname, W, H, D, boxes, hbb=True, pic_path=pic_path, img_source=img_source,
                 img_resolution=img_resolution,json_path = jsonPath)
        print('正在处理%s' % filenames[i])
#
# import cv2
# pic_path = 'D:\\tianzhibei\\testimset\\images\\16-11.jpg'
# im = cv2.imread(pic_path)  # 读取相应的图片
# (H, W, D) = im.shape  # 返回样本的大小